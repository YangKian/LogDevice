...
class PrometheusEnumerationCallback : public Stats::EnumerationCallbacks {
 public:
  ...
  void stat(const std::string& name,
    ...
    updateCounter(name,
                  {{"message_type", messageTypeNames()[message_type]}},
  ...
  void stat(const std::string& name, RequestType type, int64_t val) override {
    updateCounter(name,
                  {{"request_type", requestTypeNames[type]}},
                  static_cast<double>(val));
  }
  ...
  void stat(const std::string& name,
            ...
            int64_t val) override {
    updateCounter(name,
                  {{"storage_task_type", toString(type)}},
                  static_cast<double>(val));
  }
  ...
  void stat(const std::string& name,
    ...
    updateCounter(name,
                  {{"worker_id", std::to_string(worker_id.val())}},
                  static_cast<double>(val));
  }
  // Per-log stats.
...
void PrometheusStatsPublisher::publish(
  ...
  for (const auto& c : current) {
    auto cb = PrometheusEnumerationCallback(
        this, c->params->get()->getStatsSetName());
    c->enumerate(&cb);
  }
}
