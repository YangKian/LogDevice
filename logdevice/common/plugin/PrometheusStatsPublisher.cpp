...
class PrometheusEnumerationCallback : public Stats::EnumerationCallbacks {
 public:
  PrometheusEnumerationCallback(PrometheusStatsPublisher* publisher,
      ...
      : publisher_(publisher), stats_set_name_(stats_set_name) {}
  ...
  void stat(const std::string& name, int64_t val) override {
    updateCounter(name, {}, static_cast<double>(val));
  }
  ...
  void stat(const std::string& name,
    ...
    updateCounter(
        name, {{"message_type", messageTypeNames()[message_type]}}, static_cast<double>(val));
  }
  // Per-shard stats.
  void stat(const std::string& name,
            shard_index_t shard,
            int64_t val) override {
    updateCounter(name, {{"shard_index", std::to_string(shard)}}, static_cast<double>(val));
  }
  ...
  void statWithTag(const std::string& name,
                           ...
                           int64_t val) override {
    updateCounter(name, {{"monitoring_tag", tag}}, static_cast<double>(val));
  }
  // Per-request-type stats.
  void stat(const std::string& name, RequestType type, int64_t val) override {
    updateCounter(name, {{"request_type", requestTypeNames[type]}}, static_cast<double>(val));
  }
  // Per-storage-task-type stats.
  void stat(const std::string& name,
            StorageTaskType type,
            int64_t val) override {
    updateCounter(name, {{"storage_task_type", toString(type)}}, static_cast<double>(val));
  }
  // Per-worker stats (only for workers of type GENERAL).
  void stat(const std::string& name,
            worker_id_t worker_id,
            uint64_t val) override {
    updateCounter(name, {{"worker_id", std::to_string(worker_id.val())}}, static_cast<double>(val));
  }
  // Per-log stats.
  void stat(const char* name,
            const std::string& log_group,
            int64_t val) override {
    updateCounter(name, {{"log_group", log_group}}, static_cast<double>(val));
  }
  // Simple histograms.
  void histogram(const std::string& name,
                 const HistogramInterface& hist) override {
    updateCounter(name, {{"percentile", "50"}}, static_cast<double>(hist.estimatePercentile(0.5)));
    updateCounter(name, {{"percentile", "90"}}, static_cast<double>(hist.estimatePercentile(0.9)));
    updateCounter(name, {{"percentile", "99"}}, static_cast<double>(hist.estimatePercentile(0.99)));
    updateCounter(name, {{"percentile", "max"}}, static_cast<double>(hist.estimatePercentile(1)));
  }
  ...
  void histogram(const std::string& name,
    ...
    updateCounter(
        ...
        static_cast<double>(hist.estimatePercentile(0.5)));
    updateCounter(
        name,
        {{"shard_index", std::to_string(shard)}, {"percentile", "90"}},
        static_cast<double>(hist.estimatePercentile(0.9)));
    updateCounter(
        name,
        {{"shard_index", std::to_string(shard)}, {"percentile", "99"}},
        static_cast<double>(hist.estimatePercentile(0.99)));
    updateCounter(
        name,
        {{"shard_index", std::to_string(shard)}, {"percentile", "max"}},
        static_cast<double>(hist.estimatePercentile(1)));
  }
  ...
  void histogramWithTag(const std::string& name,
    ...
    updateCounter(
        name,
        {{"monitoring_tag", tag}, {"percentile", "50"}},
        static_cast<double>(hist.estimatePercentile(0.5)));
    updateCounter(
        name,
        {{"monitoring_tag", tag}, {"percentile", "90"}},
        static_cast<double>(hist.estimatePercentile(0.9)));
    updateCounter(
        name,
        {{"monitoring_tag", tag}, {"percentile", "99"}},
        static_cast<double>(hist.estimatePercentile(0.99)));
    updateCounter(
        name,
        {{"monitoring_tag", tag}, {"percentile", "max"}},
        static_cast<double>(hist.estimatePercentile(1)));
  }

 private:
  PrometheusStatsPublisher* publisher_;
  std::string stats_set_name_;
};
} // namespace
